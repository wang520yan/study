...
# r.set('it_user:id:2:name','zhangsan')
print r.get('name')

# 连接池
pool = redis.ConnectionPool(host='127.0.0.1', port=6379)
r = redis.Redis(connection_pool=pool)
# r.set('it_user:id:2:age', '23')
print r.get('it_user:id:2:age')

# 管道 一次请求中指定多个命令
pipe = r.pipeline(transaction=True)
r.set('it_user:id:3:name','lisi')
r.set('it_user:id:3:age', '100')
print r.get('it_user:id:3:name')
pipe.execute()

r.set(name=None,value=None,ex=None,px=None,nx=None,xx=None)
'''参数：
...
'''
r.setex(name=None,value=None,time=None)
# 设置过期时间（秒）
r.psetex(name=None,time_ms=None,value=None)
# 设置过期时间（毫秒）
...
r.incrbyfloat(name=None, amount=1.0)

# 类似 incr() 自增,amount为自增数(浮点数)
r.decr(name=None, amount=1)

# 自减name对应的值,当name不存在时,则创建name＝amount，否则，则自减，amount为自增数(整数)
r.append(name=None, value=None)
...
r.hset(name=None, key=None, value=None)

# name对应的hash中设置一个键值对（不存在，则创建，否则，修改）
r.hset("dic_name", "a1", "aa")
r.hget(name=None, key=None)
...
r.hmset(name=None, mapping=None)

# 在name对应的hash中批量设置键值对,mapping:字典
dic = {"a1": "aa", "b1": "bb"}
r.hmset("dic_name", dic)
print(r.hget("dic_name", "b1"))  # 输出:bb
r.hmget(name=None, keys=None)
...
r.hdel("dic_name", "a1")
r.hincrby(name=None, key=None, amount=1)

# 自增hash中key对应的值，不存在则创建key=amount(amount为整数)
print(r.hincrby("demo", "a", amount=2))
r.hincrbyfloat(name=None, key=None, amount=1.0)
...
r.ltrim(name=None, start=0, end=10)

# 移除列表内没有在该索引之内的值
r.ltrim("list_name", 0, 2)
r.rpoplpush(src=None, dst=None)

# 从一个列表取出最右边的元素，同时将其添加至另一个列表的最左边
# src 要取数据的列表
# dst 要添加数据的列表
r.brpoplpush(src=None, dst=None, timeout=0)

# 同rpoplpush，多了个timeout, timeout：取数据的列表没元素后的阻塞时间，0为一直阻塞
# r.brpoplpush("list_name", "list_name1", timeout=0)
r.blpop(keys=None, timeout=None)
